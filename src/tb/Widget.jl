...
function step_sim_blk(trx)
    @unpack param, bist, drv, ch, splr, eb, clkgen, cdr = trx
    ...
    # Dynamic sub-block scheduling: generate candidate sampling instants first,
    # check readiness against the actual times (including Φ0 with osr_rx scaling,
    # skew, and jitter), then sample.  Generating times before the readiness
    # check ensures jitter is not regenerated for the same sub-block.
    subblk_count = 0
    while true
        # Save clkgen state so we can roll back if the buffer is not ready.
        saved_pi_code_prev = clkgen.pi_code_prev
        saved_pi_wrap_ui   = clkgen.pi_wrap_ui
        prev_Φo_len        = length(clkgen.Φo)

        # Generate candidate sampling instants (Φ0 uses osr_rx, skew/jitter
        # use osr; all expressed as absolute TX-grid sample times).
        clkgen_pi_itp_top!(clkgen, eb, pi_code=cdr.pi_code)

        # Check whether the elastic buffer holds every sample needed to
        # interpolate at the actual generated times (including the k0+1 sample
        # required by linear interpolation).
        if !eb_can_read_times(eb, clkgen.Φo_subblk)
            # Buffer not ready: roll back clkgen state and stop scheduling.
            clkgen.pi_code_prev = saved_pi_code_prev
            clkgen.pi_wrap_ui   = saved_pi_wrap_ui
            resize!(clkgen.Φo, prev_Φo_len)
            break
        end

        # Buffer ready: sample at the already-generated Φo_subblk (no jitter
        # regeneration) and run the rest of the sub-block pipeline.
...
function step_sim_subblk(trx, blk_idx)
    ...
    # Timing (clkgen.Φo_subblk) already generated by the scheduling loop in
    # step_sim_blk; do not call clkgen_pi_itp_top! here to avoid regenerating
    # jitter.
