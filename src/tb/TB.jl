...
function init_trx()
    ...
    cdr = TrxStruct.Cdr(
                ...
                kp = 1/2^6,
                # ki bumped from 1/2^14 to 1/2^12 so the integrator can track
                # the default freq_offset_ppm = 100 (drift = 0.205 pi_codes /
                # sub-block).  With 1/2^14 the loop can only track |ppm| ≲ 30
                # and BER goes to 0.5 at ppm = 100.  For ppm = 0 this gain is
                # still well within the stable range.
                ki = 1/2^12,
    ...
    # RX window: holds the analog waveform across TX-block boundaries so
    # that the RX sampler can interpolate at RX-clock instants which drift
    # relative to the TX simulation grid.
    ...
    return (;param, bist, drv, ch, clkgen, splr, dslc, eslc, cdr, adpt, rxw, wvfm)
end

function sim_subblk(trx, blk_idx)
    @unpack param, bist, clkgen, splr, rxw = trx
    @unpack dslc, eslc, cdr, adpt, wvfm = trx

    param.cur_subblk = blk_idx

    # Φo_subblk was generated by the caller (sim_blk) BEFORE this call, and
    # rxw_covers has already confirmed readiness; the caller has also
    # appended to clkgen.Φo.  Here we only sample, slice, and update CDR.
...
function sim_blk(trx, blk_idx)
    @unpack param, bist, drv, ch, clkgen, splr, rxw = trx
    @unpack dslc, eslc, cdr, adpt, wvfm = trx
    ...
    # Apply RX sampler bandwidth filter and hand the filtered block to the
