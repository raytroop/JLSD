...
function sim_blk(trx, blk_idx)
    ...
    # all timing perturbations: Φ0 scaled by osr_rx, Φskew, Φrj), then check
    # whether those exact times are available in the elastic buffer.  The same
    # Φo_subblk is reused for sampling so that random jitter is not regenerated
    # between the readiness check and the actual read.
    subblk_count = 0
    while true
