...
@kwdef mutable struct Bist
    ...
    # Variable-length under elastic-buffer scheduling: the per-block
    # symbol count varies with `freq_offset_ppm` so we use a resizable
    # Vector instead of the original fixed-size CircularBuffer.
