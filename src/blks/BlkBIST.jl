...
function ber_checker_top!(bist)
    ...
    # Under freq_offset_ppm != 0 the RX-symbol count per TX block varies
    # (e.g. 31 / 32 / 33 for subblk_size=32).  Resize working buffers to
    # match nsym; clear Si at the end so the next block starts fresh.
    ...
    if bist.chk_lock_status
        ...
        for n = 1:nbits_rcvd
            ...
            if bist.chk_lock_cnt == bist.chk_lock_cnt_threshold
                ...
                println("prbs locked")
                # Count the trailing bits after lock in this same block; the
                # original code threw them away by reusing bist_prbs_gen only
                # to advance the seed.
